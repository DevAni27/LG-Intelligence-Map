...
class WHOService{
  ...
  List<GlobalEvent> _parseWHOData(dynamic data){
    ...
    for(final item in items){
      ...
      events.add(GlobalEvent(
        ...
        sourceUrl: 'https://www.who.int/emergencies/disease-outbreak-news/item/$urlName',
      ));
  ...
  EventSeverity _estimateSeverity(String summary){
    ...
    if(text.contains("death") || text.contains("fatal") || text.contains("killed")){
  ...
  String cleanSummary(String summary){
    ...
    final truncated = cleaned.length > 300 ? cleaned.substring(0, 300) : cleaned;
    final lastDot = truncated.lastIndexOf('.');
  
    return lastDot != -1 ? truncated.substring(0, lastDot + 1).trim() : truncated;
  }
}
